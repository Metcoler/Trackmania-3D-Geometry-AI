...
Táto kapitola zavádza pojmy, ktoré budú použité v ďalších častiach práce. Cieľom ešte nie je opisovať implementáciu ani výsledky experimentov, ale pripraviť spoločný slovník: agent, prostredie, pozorovanie, akcia, politika správania, odmena, neurónová sieť a optimalizačný algoritmus.

Text postupuje od všeobecného k špecifickému. Najprv zavedieme rozhodovanie agenta v prostredí, potom hodnotenie správania, neurónové siete a metódy, ktoré hľadajú vhodné parametre rozhodovacej funkcie.
...
Z tohto dôvodu je vhodné odlíšiť pravidlový systém od systému, ktorý sa učí. Pravidlový systém má správanie ručne opísané vopred, napríklad podmienkami a reakciami na prekážky. Je čitateľný a kontrolovateľný, ale v zložitejšom prostredí môže byť krehký. Učiaci sa systém má namiesto pevne napísaných pravidiel parametre, ktoré sa menia podľa dát alebo hodnotenia správania. Tým sa znižuje potreba ručného návrhu, no rastie význam správneho učenia a hodnotenia.

Autonómne rozhodovanie je preto prirodzene sekvenčný problém. Agent pozoruje prostredie, zvolí akciu, prostredie sa zmení a proces pokračuje. Rovnaká akcia môže mať iný význam pri nízkej a vysokej rýchlosti alebo v inej časti zákruty. Už tu vzniká hlavná otázka práce: ako nájsť rozhodovaciu funkciu, ktorá v každom kroku vyberie vhodnú akciu.
...
\]
potom hodnotenie vhodnosti môžeme chápať ako funkciu
\[
    F(\tau),
\]
ktorá celej trajektórii priradí jedno hodnotenie.

Pri učení s učiteľom sa naopak častejšie minimalizuje stratová funkcia (angl.~\emph{loss}). Stratová funkcia meria rozdiel medzi výstupom modelu a požadovaným cieľom v dátach. Kým odmena alebo hodnotenie vhodnosti býva formulované tak, aby väčšia hodnota znamenala lepšie správanie, stratová funkcia sa zvyčajne minimalizuje. Napriek rozdielnemu znamienku ide stále o rovnaký typ otázky: ako číselne vyjadriť, že jeden kandidát je lepší než iný.

\paragraph{Jedno číslo a viac cieľov.}
Ak je hodnotenie vyjadrené jedným číslom, porovnávanie agentov je jednoduché: vyššia hodnota znamená lepšieho kandidáta. To je praktické pre algoritmy, ktoré potrebujú jedincov zoradiť, vybrať najlepších alebo vypočítať gradient.

V riadiacich úlohách však často sledujeme viac cieľov naraz: agent sa má dostať do cieľa, byť rýchly, stabilný a nemá riskovať nežiaduce stavy. Tieto ciele môžu byť v konflikte. Rýchle správanie býva riskantnejšie a bezpečné správanie pomalšie.
...
\]
kde $S$ je výsledné skóre a $w_i$ sú váhy metrík. Váhy môžu vyjadrovať priority, ale sú citlivé na návrh. Ak nezodpovedajú skutočnému cieľu, agent môže optimalizovať nesprávny kompromis.
...
Návrh hodnotenia je preto jednou z najdôležitejších častí práce s autonómnym agentom. Agent neoptimalizuje náš zámer, ale formálne zadané kritérium. Ak sa toto kritérium líši od skutočného cieľa, môže vzniknúť správanie, ktoré je podľa vzorca dobré, ale prakticky nežiaduce. Tento problém sa objavuje pri návrhu odmien aj pri tvarovaní odmeny (angl.~\emph{reward shaping}) \cite{ng1999shaping}; literatúra o nesprávne navrhnutých odmenách zároveň upozorňuje na riziko správania, ktoré optimalizuje nesprávny zámer návrhára \cite{knox2023reward_misdesign}.
...
Aktivačná funkcia zavádza do siete nelinearitu. Bez nej by skladanie viacerých lineárnych vrstiev bolo stále iba jednou lineárnou transformáciou, čo je pre riadenie v nelineárnom prostredí príliš slabé. Medzi základné nelineárne funkcie patria napríklad sigmoid,
\[
...
\]
alebo hyperbolický tangens $\tanh(z)$, ktorý výstup obmedzuje do intervalu $[-1, 1]$. Výber aktivačnej funkcie ovplyvňuje, aké transformácie vie sieť vyjadriť a ako sa jej parametre počas učenia menia \cite{goodfellow2016deep}.
...
Pri pevnej architektúre je počet váh a posunov konečný. Všetky matice a vektory parametrov preto môžeme formálne sploštiť do jedného dlhého vektora
\[
    \tilde{\theta}
    =
    ...
    \in \mathbb{R}^{d},
\]
kde $\operatorname{vec}$ označuje sploštenie parametrov a $d$ je ich celkový počet. Jedno nastavenie siete je teda bod v priestore $\mathbb{R}^{d}$ a učenie znamená hľadanie bodu, pri ktorom sieť produkuje vhodné výstupy.

Táto formulácia oddeľuje tvar modelu od hľadania jeho parametrov. Neurónová sieť ešte nie je naučené správanie, ale rodina funkcií, z ktorej tréning vyberá jednu konkrétnu funkciu.
...
Učenie s učiteľom (angl.~\emph{supervised learning}) vychádza z dát, v ktorých ku každému vstupu poznáme požadovaný výstup. Takúto dátovú množinu môžeme zapísať ako
\[
    \mathcal{D} = \{(\mathbf{x}_i, \mathbf{y}_i)\}_{i=1}^{N},
\]
kde $\mathbf{x}_i$ je vstupný príklad, $\mathbf{y}_i$ je cieľová hodnota a $N$ je počet príkladov v dátovej množine. Model $f_\theta$ s parametrami $\theta$ vytvorí pre vstup $\mathbf{x}_i$ predikciu $f_\theta(\mathbf{x}_i)$ a cieľom učenia je, aby sa táto predikcia čo najviac podobala cieľovej hodnote $\mathbf{y}_i$.

Túto podobnosť vyjadruje stratová funkcia. Priemernú stratu na dátovej množine môžeme zapísať napríklad ako
\[
    \mathcal{L}(\theta) =
    \frac{1}{N}\sum_{i=1}^{N}
...
Učenie napodobňovaním (angl.~\emph{imitation learning}) je špeciálny prípad, v ktorom sa model učí správanie od experta. Expertom môže byť človek alebo iný systém, ktorý v danom prostredí vykonáva požadované akcie. V jednoduchom variante, ktorý sa nazýva klonovanie správania (angl.~\emph{behavior cloning}), zbierame demonštrácie experta a model trénujeme podobne ako pri učení s učiteľom. Dátovú množinu expertných ukážok môžeme zapísať ako
\[
    \mathcal{D}_E = \{(o_i, a_i^E)\}_{i=1}^{N},
\]
kde $o_i$ je pozorovanie a $a_i^E$ je akcia experta v tomto pozorovaní.

V úlohách riadenia sa model učí mapovanie z pozorovania na akciu experta. Tento smer má dlhú históriu: systém ALVINN sa učil riadiť vozidlo z ľudských ukážok \cite{pomerleau1989alvinn} a neskoršie prístupy ukázali podobnú myšlienku pri riadení z obrazu \cite{bojarski2016endtoend}. Obmedzenie je v tom, že model vidí najmä stavy navštívené expertom. Po malej chybe v uzavretej slučke sa môže dostať mimo trénovaciu distribúciu, čo je jedna z motivácií metód ako DAgger \cite{ross2011dagger}.

\paragraph{Učenie bez učiteľa.}
Učenie bez učiteľa (angl.~\emph{unsupervised learning}) pracuje s dátami bez priamo zadaného správneho výstupu. Namiesto mapovania na označenie hľadá štruktúru v dátach, napríklad zhluky, nižšiu dimenziu alebo skryté príznaky. V tejto práci nejde o hlavný spôsob tréningu, ale dopĺňa širší kontext strojového učenia.
...
Genetický algoritmus (angl.~\emph{genetic algorithm}) je optimalizačná metóda inšpirovaná evolúciou. Namiesto toho, aby upravoval jeden model postupným gradientným učením, pracuje s populáciou kandidátov. Každý kandidát sa vyhodnotí pomocou hodnotenia vhodnosti, najlepší kandidáti sa vyberú ako rodičia a z nich vzniká nová generácia pomocou kríženia a mutácie \cite{goldberg1989ga}. Tento proces sa opakuje, až kým populácia nenájde dostatočne dobré riešenie alebo sa nevyčerpá výpočtový rozpočet.
...
\paragraph{Funkcia vhodnosti.}
Kvalitu kandidáta vyjadruje hodnotenie vhodnosti (angl.~\emph{fitness}). Pre kandidáta $x_i^{(g)}$ ho môžeme zapísať ako
\[
    F(x_i^{(g)}).
\]
Funkcia $F$ priraďuje kandidátovi číselné hodnotenie. Vyššia hodnota zvyčajne znamená lepšie riešenie, hoci konkrétna interpretácia závisí od úlohy. V riadiacich úlohách môže funkcia vhodnosti hodnotiť celú trajektóriu agenta, nie iba jeden izolovaný krok. To je praktické, pretože kvalita správania sa často ukáže až po dlhšej postupnosti rozhodnutí.

Návrh funkcie vhodnosti je rovnako dôležitý ako návrh odmeny v učení posilňovaním. Ak toto hodnotenie nevyjadruje skutočný cieľ úlohy, algoritmus bude dôsledne vyberať kandidátov, ktorí sú dobrí podľa nesprávneho kritéria. Genetický algoritmus teda nerieši problém definície cieľa. Poskytuje spôsob, ako cieľ optimalizovať, keď ho už vieme vyhodnotiť.

Obrázok~\ref{fig:ga_initial_population} ukazuje zjednodušenú predstavu populácie. Farba štvorca tu nereprezentuje kvalitu jedinca, ale iba jeho vlastnosti alebo genotyp. Kvalitu správania vyjadruje až samostatné hodnotenie vhodnosti nad štvorcom.
...
\begin{figure}[htbp]
    ...
    \caption[Selekcia jedincov]{Selekcia jedincov podľa hodnotenia vhodnosti.}
...
Pri genetických algoritmoch sa často hovorí o prieskume a dolaďovaní. Prieskum znamená hľadanie nových oblastí priestoru riešení. Dolaďovanie znamená menšie úpravy už sľubných riešení. Veľká populácia, vyššia mutácia alebo slabší selekčný tlak môžu podporovať prieskum. Menšia mutácia a silnejší výber najlepších kandidátov môžu podporovať dolaďovanie.
...
Pri učení a vyhodnocovaní agentov nestačí mať iba politiku a hodnotenie. Potrebujeme aj prostredie, v ktorom možno politiku opakovane spustiť, pozorovať dôsledky jej akcií a zaznamenať trajektóriu spolu s metrikami.
...
Pre túto prácu sú podstatné najmä opakovateľnosť a časovanie. Pri opakovateľnosti rozlišujeme \textbf{deterministické} a \textbf{stochastické alebo prakticky variabilné} prostredie. V deterministickom prostredí rovnaký stav a rovnaká akcia vedú k rovnakému ďalšiemu stavu:
\[
    s_{t+1} = f_{\text{sim}}(s_t, a_t),
\]
kde $f_{\text{sim}}$ je prechodová funkcia simulácie. Ak výsledok závisí od šumu, počiatočných podmienok, numeriky alebo časovania, prechod opisujeme pravdepodobnostne:
\[
    s_{t+1} \sim P_{\text{sim}}(\cdot \mid s_t, a_t).
\]
Deterministické prostredie uľahčuje opakovanie experimentov; variabilné prostredie je náročnejšie na interpretáciu, ale lepšie odhaľuje krehkosť správania.

Druhou otázkou je časovanie. V \textbf{synchrónnom} prostredí agent vykoná akciu, prostredie spraví jeden krok a až potom vznikne ďalšie pozorovanie. V \textbf{asynchrónnom} prostredí alebo prostredí bežiacom v reálnom čase môže medzi rozhodnutiami uplynúť rôzny čas. Rovnaká akcia preto nemusí mať rovnaký účinok, ak pôsobí dlhšie, kratšie alebo v inom okamihu fyzikálneho výpočtu.
...
Z pohľadu teórie je Trackmania úloha sekvenčného rozhodovania v reálnom čase. Auto sa pohybuje priebežne a každá akcia mení stav, z ktorého sa bude rozhodovať neskôr. Aj malá odchýlka v brzdení či natočení môže po niekoľkých sekundách znamenať inú stopu na trati.
...
Práve tento konflikt robí z pretekárskych hier užitočné prostredie pre učiacich sa agentov. Úloha má jasne merateľný cieľ, no správanie treba hodnotiť cez celý prejazd, jeho stabilitu, čas a schopnosť neopustiť použiteľnú trajektóriu \cite{offline_mania2024}.

Trackmania je zároveň vhodná aj na porovnanie voči človeku. Keďže výsledok jazdy je čas prejazdu, malé rozdiely sú priamo viditeľné v rebríčku. Pri krátkych technických tratiach však nemožno automaticky predpokladať, že rozdiel pol sekundy je zanedbateľný. V hornej časti rebríčkov sa hráči často odlišujú o desatiny alebo stotiny sekundy.

Túto mierku ilustrujú verejné časy hráčov na klasickej mape \emph{A01-Race} \cite{tmnf_exchange_a01_race2026, maniaexchange_api2026}. Obrázok~\ref{fig:trackmania_a01_human_times} zobrazuje horných 1000 časov z rebríčka tejto mapy. Nejde o štatistický model všetkých hráčov, ale o praktický pohľad na to, ako tesne sú pri takejto krátkej trati zoradené dobré prejazdy. Vyhladená krivka v grafe slúži iba na lepšiu čitateľnosť histogramu; netvrdíme, že časy majú normálne rozdelenie.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{images/theory/theory_trackmania_a01_human_times.pdf}
    \caption[Ľudské časy na mape A01-Race]{Rozdelenie horných 1000 časov z verejného rebríčka mapy \emph{A01-Race}. Histogram a vyhladená krivka slúžia iba na vizuálne zobrazenie vzorky; netvrdíme, že časy majú normálne rozdelenie. Dáta boli odvodené z verejne dostupných záznamov k tejto mape \cite{tmnf_exchange_a01_race2026, maniaexchange_api2026}.}
    \label{fig:trackmania_a01_human_times}
\end{figure}

V tejto vzorke bol najlepší čas približne \(23{,}77\) sekundy. Medzi prvým a desiatym časom bol rozdiel iba približne \(0{,}07\) sekundy. Horných 100 časov sa zmestilo do približne \(0{,}22\) sekundy, horných 500 do \(0{,}41\) sekundy a horných 1000 do \(0{,}71\) sekundy. Pointa je jednoduchá: pri krátkej technickej trati môže aj rozdiel niekoľkých desatín sekundy znamenať viditeľný posun v poradí. Preto neskôr nebudeme rozdiel \(0{,}5\) sekundy čítať ako maličkosť, ale ako rozdiel, ktorý má v Trackmanii praktický význam.
