...
VARIANTS = [
    ("exp00_base_fixed100", "Baseline", "#425466"),
    ("exp01_mutation_decay_fixed100", "Decay\nfrom start", "#8c6bb1"),
    ("exp01b_mutation_decay_first_finish_fixed100", "Decay\nafter first finish", "#5b8dd6"),
    ("exp02_both_mirrors_fixed100", "Both\nmirrors", "#2f9e8f"),
    ("exp03_mirror_prob_050_fixed100", "Random\nmirroring", "#d28f2d"),
    ("exp05a_variable_tick_no_cache_control", "Variable timing\nwithout cache", "#c55a5a"),
    ("exp05b_variable_tick_elite_cache", "Variable timing\nwith elite cache", "#2f8f5b"),
]

FOCUS_DECAY = [
    ("exp00_base_fixed100", "Baseline", "#425466"),
    ("exp01b_mutation_decay_first_finish_fixed100", "Decay after first finish", "#5b8dd6"),
]

FOCUS_CACHE = [
    ("exp05a_variable_tick_no_cache_control", "Without elite cache", "#c55a5a"),
    ("exp05b_variable_tick_elite_cache", "With elite cache", "#2f8f5b"),
]
...
def plot_summary(summary: pd.DataFrame) -> None:
    ...
    axes[0].set_title("Best finished time")
    ...
    for i, value in enumerate(best_time):
        if math.isfinite(value):
            ...
            axes[0].text(i, 0.45, "no\nfinish", ha="center", va="bottom", fontsize=8)
    ...
    axes[1].set_title("Finishing individuals")
    ...
    axes[2].set_title("Late stability")
...
def plot_focus(summary: pd.DataFrame) -> None:
    ...
    def draw_column(col: int, experiments: list[tuple[str, str, str]], title: str) -> None:
        ...
        progress_ax.set_ylabel("Best progress [%]")
        progress_ax.set_ylim(-3, 103)
        finish_ax.set_ylabel("Finishing individuals")
        finish_ax.set_xlabel("Generation")

    draw_column(0, FOCUS_DECAY, "Mutation decay")
    draw_column(1, FOCUS_CACHE, "Elite cache")
